...
- `ServiceBootstrap` launches the service with telemetry arguments from `UnityCodeAgentSettings`. `TelemetryMode.File` passes `--EnableTelemetry=true` and optional `--CliTelemetryFilePath`, without `--OtlpEndpoint`; `TelemetryMode.OtlpEndpoint` passes `--OtlpEndpoint` and disables file path arguments.
...
- Telemetry behavior:
  - Prefer `TelemetryMode.File` or explicit `CliTelemetryFilePath` so the runner can parse SDK JSONL locally.
